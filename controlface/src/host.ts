// The ControlFace HTTP host: the composition that serves the two faces over their transports.
// SSE (`/gup`) binds to the ControlFace boundary (render/drive); MCP (`/mcp`) binds to the AgentFace
// projection. One runtime, two audiences — the trust boundary is which face each route exposes. The
