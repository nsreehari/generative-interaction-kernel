...
const functions = (() => {
    ...
    function evaluateMatcher(matcher, str) {
        ...
        if(isPromise(result)) {
            throw new Error('Sync JSONata port encountered Promise-returning matcher');
        }
                ...
                if (typeof matches !== 'undefined') {
                    while (typeof matches !== 'undefined' && (typeof limit === 'undefined' || count < limit)) {
                        ...
                        if (isPromise(replacedWith)) {
                            throw new Error('Sync JSONata port encountered Promise-returning replacer');
                        }
...
var jsonata = (function() {
    ...
    function applyInner(proc, args, input, environment) {
        ...
        try {
            ...
            if (isLambda(proc)) {
                ...
                if (isPromise(result)) {
                    throw new Error('Sync JSONata port encountered Promise-returning JSONata function');
                }
                ...
                if (isPromise(result)) {
                    throw new Error('Sync JSONata port encountered Promise-returning host function');
                }
            } else {
                throw {
                    code: "T1006",
    ...
    function applyNativeFunction(proc, env) {
        ...
        if (isPromise(result)) {
            throw new Error('Sync JSONata port encountered Promise-returning native function');
        }
        return result;
    }
