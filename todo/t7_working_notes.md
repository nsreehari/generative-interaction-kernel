...
The worker in `samples/apps/node-host/continuity-worker.ts` has no kernel, store, or face reference. It
communicates only through HTTP MCP endpoints and can therefore run in another process or host.

`samples/apps/node-host/service.test.ts` proves:
...
```bash
npx vitest run --project samples samples/apps/node-host/service.test.ts
```
