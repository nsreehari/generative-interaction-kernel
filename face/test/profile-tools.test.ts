...
const profile: Profile = {
  ...
  version: "0.1",
  layers: [
    { id: "spec", kind: "spec", schema: "spec.schema", description: "the demo spec layer" },
    { id: "view", kind: "view" },
  ],
  recipes: [],
  authoring: {
...
test("materializes one McpTool per declaration, carrying agentSafe + input schema", () => {
  ...
  assert.deepEqual(tools.map((t) => t.name).sort(), ["describeSpec", "projectView", "validateSpec"]);
...
test("validate runs structural validator then named checks; checks skipped on structural error", () => {
  ...
  assert.deepEqual(validate.handler({ spec: {} }), {
    ok: false,
    errors: [{ detail: "spec.kind (string) is required" }],
    warnings: [],
  });
