...
test("host registry hydrates portfolio holdings from browser storage", () => {
  const values = new Map<string, string>([[
    portfolioStateStorageKey,
    JSON.stringify({ portfolio: { holdings: { MSFT: { ticker: "MSFT", quantity: 7, costBasis: 405 } } } }),
  ]]);
  ...
  try {
    const entry = createHostRegistry().get("portfolio-tracker");
    assert.equal(entry?.kind, "bundle");
    if (!entry || entry.kind !== "bundle") throw new Error("portfolio-tracker bundle is unavailable");
    const portfolio = entry.make().state?.portfolio as Record<string, unknown>;
    assert.deepEqual(portfolio.holdings, {
      MSFT: { ticker: "MSFT", quantity: 7, costBasis: 405 },
    });
  } finally {
