...
const useStyles = makeStyles({
  ...
  workspaceFullscreen: { gridTemplateRows: "minmax(0, 1fr)" },
  ...
  columnsFullscreen: { gridTemplateColumns: "minmax(0, 1fr)", gridTemplateRows: "minmax(0, 1fr)", borderTop: 0, overflow: "hidden" },
  ...
  fullscreenExit: { position: "absolute", zIndex: 10, top: tokens.spacingVerticalM, right: tokens.spacingHorizontalM, backgroundColor: tokens.colorNeutralBackground1, boxShadow: tokens.shadow8 },
...
const WorkspaceView: ProjectionView = ({ node, children }) => {
  ...
  const fullscreen = node.props.fullscreen === true;
  const cells = childrenById(children);
  return <main className={`${styles.workspace} ${fullscreen ? styles.workspaceFullscreen : ""}`}>
    {fullscreen ? null : <header className={styles.topbar}>
...
const ReportView: ProjectionView = ({ node, children }) => {
  ...
  return <section className={`${styles.report} ${fullscreen ? styles.reportFullscreen : ""}`} style={fullscreen ? { gridTemplateRows: "0 minmax(0, 1fr)" } : undefined}>
    {fullscreen ? <div className={styles.fullscreenExit}>{cells.get("incident-exit-fullscreen")}</div> : <header className={styles.reportHeader}>
      ...
      <div className={styles.reportActions}>{cells.get("incident-view-fullscreen")}{cells.get("incident-analyze-report")}</div>
    </header>}
    {fullscreen ? <div aria-hidden="true" /> : null}<div className={styles.reportBody}>{pending ? <span role="status">Analyzing incident report...</span> : null}{node.props.error ? <MessageBar intent="error"><MessageBarBody>{String(node.props.error)}</MessageBarBody></MessageBar> : null}{hasModel
