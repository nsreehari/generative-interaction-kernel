...
export const useStyles = makeStyles({
  ...
  workColumn: { minWidth: 0, minHeight: 0, height: "100%", padding: `12px clamp(16px, 3vw, 40px) clamp(18px, 3vw, 36px)`, overflow: "hidden", "@media (max-width: 1040px)": { height: "auto", padding: `10px 14px 18px`, overflow: "visible" } },
  ...
  sharedViewport: { minHeight: 0, overflow: "auto", padding: `clamp(18px, 3vw, 32px)`, paddingBottom: "calc(clamp(18px, 3vw, 32px) + 50px)", scrollPaddingBottom: "50px" },
  ...
  arrangementKanban: { display: "block", overflowX: "auto", paddingBottom: tokens.spacingVerticalS },
  kanbanBoard: { minWidth: "1120px", display: "grid", gridTemplateColumns: "repeat(5, minmax(208px, 1fr))", gap: tokens.spacingHorizontalM, alignItems: "start", "@media (max-width: 900px)": { minWidth: 0, gridTemplateColumns: "1fr" } },
  kanbanColumn: { minWidth: 0, display: "grid", alignContent: "start", gap: tokens.spacingVerticalS, padding: tokens.spacingVerticalS, borderTop: `3px solid color-mix(in srgb, var(--accent) 48%, var(--line))`, backgroundColor: "color-mix(in srgb, var(--accent) 4%, var(--panel))" },
  kanbanColumnTitle: { margin: 0, color: "var(--muted)", fontSize: tokens.fontSizeBase200, fontWeight: tokens.fontWeightBold, textTransform: "uppercase" },
  kanbanCards: { minWidth: 0, display: "grid", gap: tokens.spacingVerticalS },
