...
function componentProps(capability: string, heading: string, data: unknown): Record<string, Json> {
  ...
  if (capability === "semantic:decision") return { variant: "summary", decision: value as Json, spec: { eyebrow: heading, fields: { title: "title", summary: "summary", outcome: "outcome", rationale: "rationale", confidence: "confidence", impact: "impact", tone: "confidence" }, toneMap: { high: "affirmative", medium: "cautionary", low: "uncertain" } } };
  ...
  if (capability === "semantic:narrative") return { sections: array(data) as Json, spec: { title: heading, fields: { id: "id", heading: "heading", body: "body", parent: "parentId", order: "order", role: "role", tone: "tone" } } };
