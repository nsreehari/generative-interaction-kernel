...
export function compileScenarioBlueprint(artifact: ScenarioBlueprintArtifact): ScenarioPlan {
  ...
  for (const step of plan.steps) {
    if (!step.id.trim() || !step.title.trim()) throw new Error("Scenario steps require id and title");
    ...
    if (step.kind === "human-gate") {
      if (!step.humanBoundary) throw new Error(`Human-gate scenario step '${step.id}' requires a human boundary`);
      if (!step.command?.trim() || commands.length > 0) {
...
export function validateDemoComposition(entry: DemoCatalogEntry, scenario: ScenarioPlan, organism: OrganismDemoContract): void {
...
export function validateDemoCatalog(catalog: DemoCatalog, scenarioPlans: ReadonlyMap<string, ScenarioPlan>): DemoCatalog {
  if (!catalog || typeof catalog !== "object" || Array.isArray(catalog)) throw new Error("Demo catalog must be an object");
  ...
  for (const entry of catalog.entries) {
    ...
    if (scenario.targetBlueprintId !== entry.targetBlueprintId) {
      throw new Error(`Demo '${entry.id}' targets '${entry.targetBlueprintId}', but scenario '${scenario.id}' targets '${scenario.targetBlueprintId}'`);
    }
...
function validateDemoTarget(targetId: string, target: DemoTargetCatalogEntry): void {
  ...
  if (!target || typeof target !== "object" || Array.isArray(target)) throw new Error(`Demo target '${targetId}' must be an object`);
  ...
  for (const [field, value] of arrays) if (!Array.isArray(value)) throw new Error(`Demo target '${targetId}' requires '${field}'`);
  ...
  for (const descriptor of target.commands) {
    if (!descriptor || typeof descriptor !== "object" || !descriptor.command?.trim() || !descriptor.nodeId?.trim() || !descriptor.event?.trim()) {
      throw new Error(`Demo target '${targetId}' has an invalid command descriptor`);
    }
    if (commandNames.has(descriptor.command)) throw new Error(`Demo target '${targetId}' has duplicate command '${descriptor.command}'`);
  ...
  const validateStrings = (field: string, values: unknown[]) => {
    ...
    if (new Set(values).size !== values.length) throw new Error(`Demo target '${targetId}' has duplicate '${field}' entries`);
  };
  ...
  for (const preset of target.presentationPresets) {
    ...
    if (presetIds.has(preset.id)) throw new Error(`Demo target '${targetId}' has duplicate presentation preset '${preset.id}'`);
  ...
  for (const gate of target.humanGates) if (!commandNames.has(gate)) throw new Error(`Demo target '${targetId}' human gate '${gate}' has no command descriptor`);
}
