...
export function createIndexedDbStorage(options: IndexedDbStorageOptions = {}): DurableProvider {
  ...
  return {
    appendJournal<T>(request: { stateRef: string; journalRef: string; effectsQueueRef: string; effectsLane?: string; entry: T }) {
      const space = namespace(request.journalRef);
      ...
      return transaction("readwrite", async (store) => {
        ...
        await result(store.add({
          ...
          sequence, value: request.entry,
        }));
        const wakeId = id("runtime-wake", stateSpace, "__wake__");
        const wake = await result(store.get(wakeId)) as RecordValue | undefined;
        const processedAt = typeof wake?.processedAt === "string" ? wake.processedAt : null;
        const now = new Date().toISOString();
        const requestedAt = processedAt && now <= processedAt
          ? new Date(Date.parse(processedAt) + 1).toISOString()
          : now;
        await result(store.put({
          id: wakeId, namespace: stateSpace, kind: "runtime-wake", key: "__wake__",
          requestedAt, processedAt,
        }));
        return { id: entryId, payload: request.entry };
      });
    },

    readEngineWake(request) {
      const stateSpace = runtimeSpace(request.stateRef);
      return transaction("readonly", async (store) => {
        const wake = await result(store.get(id("runtime-wake", stateSpace, "__wake__"))) as RecordValue | undefined;
        return {
          requestedAt: typeof wake?.requestedAt === "string" ? wake.requestedAt : null,
          processedAt: typeof wake?.processedAt === "string" ? wake.processedAt : null,
        };
      });
    },
    ...
    markEngineWakeProcessed(request, processedAt) {
      ...
      return transaction("readwrite", async (store) => {
        ...
        const wake = await result(store.get(wakeId)) as RecordValue | undefined;
        if (!wake) return;
        const current = typeof wake.processedAt === "string" ? wake.processedAt : null;
        if (!current || processedAt > current) await result(store.put({ ...wake, processedAt }));
      });
    },
    ...
    acquireTransition<TState, TEvent>(request: TransitionRefs & { kernelId: string; leaseMs?: number }) {
      const stateSpace = runtimeSpace(request.stateRef);
    ...
    commitTransition(request: TransitionCommit) {
      const stateSpace = runtimeSpace(request.stateRef);
