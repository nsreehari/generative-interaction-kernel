...
import type { EngineWakeStorage, JournalStorage, LeasedTransitionStorage, QueueLaneStorage } from "./contracts";
...
type StagedQueueStorage = QueueLaneStorage & {
  stage(body: unknown): Promise<{ id: string }>;
...
export function createFilesystemDurableStorage(storage: FilesystemDurableStoragePrimitives) {
  ...
  function runtimeRefs(request: Record<string, unknown>): RuntimeRefs {
    ...
    const effectsNamespace = storage.namespaceForRef(effectsQueueRef);
    ...
    return {
      stateRef,
      effectsQueueRef,
      effectsLane: request.effectsLane === undefined || request.effectsLane === null
        ? undefined
  ...
  function refsKey(refs: TransitionRefs): string {
    return JSON.stringify([
      refs.stateRef,
      refs.journalRef,
      refs.effectsQueueRef,
      refs.effectsLane ?? null,
    ]);
  }

  async function releaseTransition(token: string): Promise<boolean> {
    ...
    return {
      async initialize(refs, initialState) {
        ...
        try {
          ...
          const current = await stateStorage.read(runtimeStateKey) as PersistedRuntimeState | null;
          if (current) {
            if (current.kernelId !== kernelId) {
              throw new Error(`Runtime state belongs to kernel ${current.kernelId}, not ${kernelId}.`);
            }
      ...
      async acquire(refs, options) {
        ...
        timer.unref?.();
        transitionTokens.set(leaseToken, { refsKey: refsKey(refs), kernelId, release, timer });

        try {
          const persisted = await storage.kvStorageForRef(refs.stateRef)
          ...
          if (persisted.kernelId !== kernelId) {
            throw new Error(`Runtime state belongs to kernel ${persisted.kernelId}, not ${kernelId}.`);
          }
          const journal = await storage.journalStorageForRef(refs.journalRef).readAfter(persisted.cursor);
          return {
            leaseToken,
            leaseExpiresAt,
      ...
      async commit(request) {
        ...
        if (!held || held.refsKey !== refsKey(refs) || held.kernelId !== kernelId) {
        ...
        try {
          ...
          const queue = storage.queueStorageForRef(request.effectsQueueRef, request.effectsLane);
          ...
          try {
            ...
            for (const message of staged) {
              await queue.discardStaged(message.id, "transition commit failed").catch(() => false);
            }
            throw error;
          }
        } finally {
      ...
      async abort(refs, leaseToken) {
        ...
        if (!held || held.refsKey !== refsKey(refs) || held.kernelId !== kernelId) return false;
  ...
  return {
    ...
    engineWakeStorage: (refs: RuntimeRefs) => {
      runtimeRefs(refs as Record<string, unknown>);
      return storage.engineWakeStorageForRef(refs.stateRef);
    },
    effectsQueueStorage: (effectsQueueRef: string, effectsLane?: string) =>
      storage.queueStorageForRef(effectsQueueRef, effectsLane),

    async appendJournalAndWake(request: Record<string, unknown>) {
      const refs = transitionRefs(request);
      if (!Object.prototype.hasOwnProperty.call(request, "entry")) throw new Error("entry is required.");
      const entry = await storage.journalStorageForRef(refs.journalRef).append(request.entry);
    ...
    async markEngineWakeProcessed(request: Record<string, unknown>) {
      ...
      await storage.engineWakeStorageForRef(refs.stateRef).markProcessed(request.processedAt);
    },
    ...
    initializeRuntime(request: Record<string, unknown>) {
      ...
      return transitionStorage(request.kernelId).initialize(refs, request.initialState);
    },
    ...
    acquireTransition(request: Record<string, unknown>) {
      ...
      const leaseMs = typeof request.leaseMs === "number" && Number.isInteger(request.leaseMs) && request.leaseMs > 0
        ? request.leaseMs
        : undefined;
    ...
    commitTransition(request: Record<string, unknown>) {
      ...
      return transitionStorage(request.kernelId).commit({
        ...
        expectedRevision: request.expectedRevision === null ? null : String(request.expectedRevision),
        previousCursor: request.previousCursor === null ? null : String(request.previousCursor),
    ...
    abortTransition(request: Record<string, unknown>) {
      ...
      return transitionStorage(request.kernelId).abort(refs, String(request.leaseToken ?? ""));
    },
  };
}
