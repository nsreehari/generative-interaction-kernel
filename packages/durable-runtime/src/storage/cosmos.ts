...
import type { QueueLeasedMessage, RuntimeRefs, TransitionRefs } from "../contracts";
import type {
...
async function readDocument<T extends RuntimeDocument>(
  container: Container,
  id: string,
  partitionKey: string,
): Promise<T | null> {
...
export function createCosmosTransitionStorage<TState = unknown, TEvent = unknown, TEffect = unknown>(
  ...
  return {
    ...
    async acquire(refs, options) {
      ...
      const lock: RuntimeLockDocument = {
        id: lockId,
        partitionKey,
        kind: "gik-transition-lock",
        leaseToken,
  ...
  return {
    ...
    async nack(messageId, leaseToken, options) {
      ...
      const next: RuntimeEffectDocument = {
        ...current,
        state: options?.dead ? "dead" : "active",
  ...
  return {
    async request() {
      ...
      await update((current) => {
        ...
        const now = new Date().toISOString();
        requestedAt = processedAt && now <= processedAt
    ...
    markProcessed(processedAt) {
      return update((current) => ({
        id, partitionKey, kind: "gik-engine-wake",
        requestedAt: current?.requestedAt ?? null,
        processedAt: !current?.processedAt || processedAt > current.processedAt
          ? processedAt
          : current.processedAt,
      }));
    },
