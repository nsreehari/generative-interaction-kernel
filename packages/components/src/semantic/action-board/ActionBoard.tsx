...
import { componentNode, defineComponent, eventContract, trialNode, type ComponentDescription, type ComponentValidationReport } from "../../shared/definition";
...
export function validateActionBoard(props: unknown): ComponentValidationReport {
  return runDeclarativeValidators([
    { kind: "ajv-schema", schema: getActionBoardSchema(), message: "Invalid semantic:action-board props", code: "semantic-action-board-schema" },
    { kind: "jsonata", expr: "($field := data.spec.fields.id; $ids := data.items.$lookup($, $field); $count($ids) = $count($distinct($ids)))", message: "Action board item ids must be unique", code: "action-board-unique-item-id" },
    { kind: "jsonata", expr: "($field := data.spec.fields.group; $groups := data.spec.columns.value; $count(data.items[$lookup($, $field) in $groups]) = $count(data.items))", message: "Action board items must reference declared column groups", code: "action-board-declared-group" },
  ], props as Json);
}
