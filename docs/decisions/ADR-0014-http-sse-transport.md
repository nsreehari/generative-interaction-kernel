...
- **Direction split fits SSE's shape.** SSE is one-directional (server → client), and GUP's
...
- **Session correlation via a header.** On `GET {path}/stream` the server generates a session id and
  returns it as the `X-GUP-Session` response header; the client echoes it on `POST {path}/event` so the
  event routes to the right connection/endpoint. No new GUP message — correlation is transport metadata.
...
- **The runtime is unchanged.** `GenUIClient` and `KernelTransportHost` are used as-is; only new
...
- **A `session`/`hello` GUP message to hand out the id.** Rejected for the same reason as ADR-0012:
...
- The platform now has a real network transport, verified end-to-end over a loopback socket: a
  `GenUIClient` onboards and round-trips an event across HTTP/SSE with zero runtime changes, and a
